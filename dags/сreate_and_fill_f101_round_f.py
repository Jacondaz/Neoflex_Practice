...
def fill_f101_round_f():
    connection = hook.get_conn()
    cursor = connection.cursor()
    log_id = start_time("Start_fill_f101")
    sql = """
    do $$
        declare
            d date := '2018-02-01';
    ...
    """
    end_time(log_id, "Finished_fill_f101")
    cursor.execute(sql)
    connection.commit()
    cursor.close()
    connection.close()
...
start >> extract_and_load >> fill_dm_account_turnover_task >> fill_dm_account_balance_31_12_2017 >> fill_dm_account_balance_f_task >> fill_f101_task >> end